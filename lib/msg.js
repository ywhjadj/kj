...
// ✅ Media Download Logic (Stable Version)
const downloadMediaMessage = async(m, filename) => {
    try {
        let msg = m.msg || m
        let type = getContentType(msg) || m.mtype
        
        // Handling ViewOnce messages
...
const sms = (conn, m, store) => {
    ...
    if (m.message) {
        ...
        if (!m.mtype) return m // Safety Check taake bot band na ho
    ...
    // Commands ke liye zaroori functions wapis add kar diye hain
